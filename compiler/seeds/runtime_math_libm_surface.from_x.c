 ...
 *   + 10 full implementations (9.2.4: math_exp_c / math_log_c / math_sqrt_c /
 *   math_cbrt_c / math_expm1_c / math_log1p_c / math_sin_c / math_cos_c /
 *   math_tan_c / math_pow_c fdlibm ports, isomorphic with
 *   the .x authority; matching math_*_impl bridges for those ten are gone).
 * Cap residual: remaining extern bridges (math_*_impl) for asin/acos/atan/atan2/erf/...
 ...
 *   + thin+rest forwards to math_*_impl + 10 full impls
 *   (exp/log/sqrt/cbrt/expm1/log1p/sin/cos/tan/pow).
...
/* math_sin_impl / math_cos_impl / math_tan_impl bridges removed (9.2.4). */
 ...
 * math_log1p_impl / math_expm1_impl / math_pow_impl bridges removed
 * (9.2.4): those seven are full fdlibm implementations on both ends. */
 ...
 * the later pragma that covers exp/log, and arm64 clang defaults to
 * fusing mul+add. */
 ...
 * lossy -E %f). PLATFORM: SHARED — FP contraction OFF. */
#pragma STDC FP_CONTRACT OFF
double math_pow_c(double base, double exp) {
  ...
  hugeu.u = 9094988921128908188ull;
  tinyu.u = 118622047889322841ull;

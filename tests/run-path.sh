...
./compiler/shuc -L . tests/path/join_basename.su -o /tmp/shuc_path_join 2>&1
exitcode=0; /tmp/shuc_path_join >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 0 ] && { echo "expected exit 0 (path_join/basename/dirname), got $exitcode"; exit 1; }

./compiler/shuc -L . tests/path/extension_stem_abs_clean.su -o /tmp/shuc_path_ext 2>&1
exitcode=0; /tmp/shuc_path_ext >/dev/null 2>&1 || exitcode=$?

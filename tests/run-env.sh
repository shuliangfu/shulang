...
# 【运行方式】在仓库根目录执行 bash tests/run-env.sh 或 ./tests/run-env.sh；可选环境变量 SHUC 指定编译器路径。
#
set -e
cd "$(dirname "$0")/.."
...
exe="/tmp/shuc_env_$$_main"
if ! $SHUC -L . tests/env/main.su -o "$exe" 2>&1; then

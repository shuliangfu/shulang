...
./compiler/shu tests/slice/main.su -o /tmp/shu_slice 2>&1
exitcode=0; /tmp/shu_slice >/dev/null 2>&1 || exitcode=$?
[ "$exitcode" -ne 20 ] && { echo "expected 20 (slice s[1]), got $exitcode"; exit 1; }

./compiler/shu -L . tests/slice/length.su -o /tmp/shu_slice_length 2>&1
...
err=$(./compiler/shu tests/slice/subscript_not_slice.su -o /tmp/shu_slice_fail 2>&1) || true
